...
**Conexões:**
- **Linha 1 (Row 1)** → Pin 7 (GPIO 4)
- **Linha 2 (Row 2)** → Pin 11 (GPIO 17)
- **Linha 3 (Row 3)** → Pin 13 (GPIO 27)
- **Linha 4 (Row 4)** → Pin 15 (GPIO 22)
- **Coluna 1 (Col 1)** → Pin 19 (GPIO 10)
- **Coluna 2 (Col 2)** → Pin 21 (GPIO 9)
- **Coluna 3 (Col 3)** → Pin 23 (GPIO 11)
...
**Configuração:** As configurações de pinagem estão centralizadas em `bomb_app/config.py`.

**Alternativa - Teclado USB:**
Se preferir usar um teclado USB numérico:
- Apenas conecte via USB.
- O sistema detecta automaticamente se estiver rodando em ambiente compatível.
...
**Conexões:**
- **Positivo** → Pin 12 (GPIO 18)
- **Negativo** → Pin 6 (GND)

**Nota:** 
- O buzzer está configurado no GPIO 18 em `bomb_app/config.py`.
